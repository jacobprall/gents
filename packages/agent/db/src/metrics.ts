...
export function recordTurnMetrics(db: AgentDB, metrics: TurnMetrics): void {
  ...
  try {
    db.db
      .prepare(
        `INSERT OR REPLACE INTO metrics (turn, model, input_tokens, output_tokens, cached_input_tokens, cost_usd, tool_calls, elapsed_ms, created_at)
         VALUES (?,?,?,?,?,?,?,?,?)`,
      )
      .run(
        metrics.turn,
...
export function getSessionMetrics(db: AgentDB): SessionMetrics {
  try {
    const row = db.db
      .prepare(
        `SELECT
